...
const double THROTTLE_MIN_PL = 1.250; // open/full throttle, max gas
const double THROTTLE_MAX_PL = 1.925; // closed/min throttle, min gas
...
void setServoLow(int servoNum){
  ...
  Servo myServo;
 if (servoNum == CHOKE_PIN){
    closedPL = CHOKE_MIN_PL;
    myServo = chokeServo;
  }
  else if (servoNum == THROTTLE_PIN){
    closedPL = THROTTLE_MAX_PL;
    myServo = throttleServo;
  }
  else if (servoNum == ESC_PIN){
    closedPL = ESC_MIN_PL;
